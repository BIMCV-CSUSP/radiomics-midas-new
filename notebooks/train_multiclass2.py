...
if __name__ == "__main__":
    ...
    if args.feature_strategy == "most_discriminant":
        ...
        # Save DataFrame with full stats (filtered to selected for brevity)
        ...
        for rank, feature_name in enumerate(top_20, start=1):
            ...
            plt.title(f"Distribution of {feature_name} by Pfirrmann class", fontsize=14)
            plt.xlabel("Pfirrmann class")
    ...
    print(f"Best model: {best_model} (Mean AUC: {best_auc:.3f})")
    print("\nMean AUC per model:")
    print(mean_auc.sort_values(ascending=False))


    # Run statistical comparison script (optional)
    ...
    postprocess_cmd = [
        ...
        "--csv_preds", preds_filepath,  # Predictions file
        "--csv_results", resultados_filepath,  # Metrics file
