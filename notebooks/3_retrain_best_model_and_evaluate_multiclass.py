...
def main():
    ...
    #     multi_class="ovr",          # pass thru to roc_auc_score
    #     needs_proba=True,           # so it calls predict_proba
    #     average="macro"
    # )
    ...
    # Calcular métricas de rendimiento en test (multiclass AUC)
    ...
    balacc_ = balanced_accuracy_score(y_test, y_pred_test)
       # Métricas generales de clasificación (multiclass)
    ...
    with open(report_path, "a", encoding="utf-8") as f_out:
        ...
        f_out.write(f"\nMejor threshold seleccionado (según F1 macro): {best_thresh:.2f}\n\n")
