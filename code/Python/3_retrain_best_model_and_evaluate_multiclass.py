...
def main():
    ...
    parser.add_argument("--label", type=str, default="Pfirrmann",
                        help="Name of the label variable (default 'Pfirrmann').")
    ...
    print("Configuration:")
    print(f"  Features CSV:       {args.csv}")
    print(f"  Variables file:     {args.variables}")
    print(f"  Model for fine-tune:{args.model}")
    ...
    output_parent_dir = os.path.join(base_dir, f"best_results_{args.label}")
    ...
    # --- LIMPIEZA IGUAL AL SCRIPT PRINCIPAL ---
    # 1. Limpieza de Pfirrmann: Eliminar nulos y vacíos
    df[args.label] = df[args.label].replace(r'^\s*$', np.nan, regex=True)
    df = df.dropna(subset=[args.label])

    # 2. Limpieza de Características: Convertir a numérico (arregla \xa0)
    cols_to_exclude = ['patient_id', 'disc', 'study_id', args.label] # 'label' por si acaso existe
    feature_cols = [c for c in df.columns if c not in cols_to_exclude]
    
    for col in feature_cols:
        df[col] = pd.to_numeric(df[col], errors='coerce')

    # 3. Eliminar filas con NaNs en características
    df = df.dropna()

    # --- DEFINICIÓN DE y (Target) ---
    # Convertimos a int para poder restar 1 y tener etiquetas 0, 1, 2, 3, 4
    y = df[args.label].astype(float).astype(int).values - 1
    
    # El resto de identificadores
    groups = df['patient_id'].values
    df['patient_id_type'] = df['patient_id'].astype(str)
    df = df.set_index('patient_id_type')
    X = df.drop(['patient_id', 'study_id', args.label, 'disc'], axis=1, errors='ignore')
    ...
    #Verificar si alguna variable del txt no está en el DataFrame
    missing_in_df = [v for v in used_vars if v not in X.columns]
    if missing_in_df:
        print(f"ADVERTENCIA: Faltan {len(missing_in_df)} variables en el CSV original.")

    # Filtrar X para que tenga el orden y las columnas exactas
    X = X[used_vars]
    X = X[used_vars]
    ...
    # Definir el directorio de salida de forma segura
    # Recomiendo usar el directorio dinámico para evitar colisiones entre runs
    output_parent_dir = Path(args.experiment_dir) / "best_results"
    
    # CRÍTICO: Crear la carpeta antes de intentar cargar o guardar nada
    os.makedirs(output_parent_dir, exist_ok=True)
    ...
    # Intentar cargar solo si existen
    if os.path.exists(estimator_path) and os.path.exists(search_path):
        print(f"  --> Loading existing model from: {estimator_path}")
    ...
    if search is None:
        ...
        # GUARDAR después de entrenar porque acabamos de comprobar que no existía
        print(f"  --> Saving new model and search results...")
        joblib.dump(search.best_estimator_, estimator_path)
        joblib.dump(search, search_path)
    
    best_estimator = search.best_estimator_
    print("\nOptimization completed.")
    print(f"  --> Best parameters: {search.best_params_}")
    print(f"  --> Files are ready at: {output_parent_dir}")
